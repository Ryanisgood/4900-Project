...
public class reducePhase {
    public void reduceNonPivot(boolean multiPoint,ArrayList< Circle> circles){
        ...
        if(multiPoint == false && circles.get(lastId).robotsSizeOnCircle()>4){
            //deal with each circle
            for(int i=0; i< circles.size();i++){
                if(i==0) {
                    //k==1 move towards to Origin
                }else{
                    //k>1 move to inner
